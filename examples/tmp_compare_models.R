...
model_on_data <- mgcv::gam(m2.price ~ s(construction.year), data = apartments)
raw_linear_model <- lm(m2.price ~ construction.year, data = apartments)
...
gam_on_lm <- DALEX::explain(raw_linear_model, label="gam_on_lm",
                             data = apartmentsTest, y = apartmentsTest$m2.price)

perf_pdp <- model_performance(gam_on_pdp)

perf_data <- model_performance(gam_on_data)
perf_lm <- model_performance(gam_on_lm)

plot(variable_importance(gam_on_pdp), variable_importance(gam_on_data), variable_importance(gam_on_lm))

plot(perf_pdp, perf_data, perf_lm, geom = "boxplot")
perf_pdp
perf_data
