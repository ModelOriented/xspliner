#' @export
xp_plot <- function(model, variable_name, plot_response = TRUE, plot_approx = TRUE, data = NULL, plot_data = FALSE) {
  ...
  response_var <- environment(model)$response_name
  xp_call <- environment(model)$xs_call[[variable_name]]
  ...
  plot_response_data <- environment(model)$xs_env_list[[variable_name]]$blackbox_response_obj
