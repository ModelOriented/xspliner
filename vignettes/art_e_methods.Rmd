...
Let's see it in in below example:
```{r message = FALSE}
library(xspliner)
...
Summary method allows you to check basic model details. See below what possibilities offers the method to xspliner model.
...
Print method works similiarly to summary. In case of passing just the model, standard `print.glm` is used.
...
*Quantitative variable*
When predictor is quantitative variable its transition is based on GAM model. For this case print uses standard `print.gam` method.
...
There are three possible object types thatcan be extracted.

## Extracting effect

Each transition is built on top of black box response data. For example the default response for quantitative variables is PDP - for qualitative ones ICE.
...
After we built transition basing on cotinuity of variable specific model is created.
...
To extract the model, use transition mehod with `type = "base"`:
...
The final result of building transition is tranformation function, that is used in final GLM model estimation.
