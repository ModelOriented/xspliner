...
The following pictures show the `ptratio` impact on `cmedv` in some random forest model based on Boston Housing Data. Below curves are obtained by the approach of the pdp and ale methods respectively.
...
If above functions had linear character, one would be tempted to approximate them with linear function. As a result it could easy to interpret how it affects the variable explained in the black box model. 
What if the function is irregular, as above? Could we approximate it with polynomials?

### Splines

Due to the large errors that occurs with approximation of functions with polynomials, the approach using spline approximation is most common solution. Splines (functions that are piecewise polynomials) have good approximating properties, in addition their form is overt so we can thus interpret the resulting function.
...
If linear model performance is similiar to black box one, it could be used as it's good interpretation.
...
So let's build random forest black box model, that we use as a base for further steps:
```{r}
boston_rf <- randomForest(cmedv ~ rm + lstat + nox, data = boston)
```

Now specify which variables should be transformed.
...
This formula is enough to build GLM model (with using default parameters and xspliner glm generating function). Nevertheless to understand deeply the approach let's go further with theory.
...
Let's verify correctness of this parameters with bare usage of `pdp::partial`:
...
Let's name approximation result as **transition**. To specify it's parameters such as spline base we can use `transition` parameter inside `xs` symbol. Similiarily to `effect`, `transition` is specified as parameters list. Possible options can be found by `??mgcv::s` (few extra options can be found in next articles.
...
To sum up, we specified formula for building GLM model, in which we transform `nox` variable with some function contructed on the following steps:
...
- we estimate PDP with use of `cubic` splines with 10 dimensional base
...
As the final model is just the GLM, the summary is also standard. One difference that we can see here is formula call.
...
Here we shortly show how plot random forest effect and transition for `nox` variable:
```{r}
plot(xp_model, "nox")
```
