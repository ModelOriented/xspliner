...
xp_plot(model_pdp, "ptratio", plot_approx = FALSE)
xp_plot(model_ale, "ptratio", plot_approx = FALSE)
```
...
```{r fig.width=2, fig.height=2, echo = FALSE, fig.show='hold'}
xp_plot(model_pdp, "ptratio")
xp_plot(model_ale, "ptratio")
```
...
```
spline_opts = <mgcv::s parameters> # named list
```
**Remark**
One of special parameters passed for `spline_opts` is `increasing`. 
...
As we already have the black box provided, we can use `xp_gam` function to build the desired model.
...
xp_plot(xp_model, "nox", data = boston, plot_data = TRUE)
xp_plot(xp_model, "nox")
```
...
`xp_gam` function offers automatical check if spline approximation is better than linear one, and use it in final model.

You may find two parameters responsible for that:
- `auto_approx` - if TRUE, automatically choose whther variable should be transformed with splines
- `compare_stat` - function of `lm` object. Defines statistic that should be used in decision between spline model and linear one. The function should have attribute `higher`. When the attribute has `"better"` value then the model with higher statistic value is chosen.
