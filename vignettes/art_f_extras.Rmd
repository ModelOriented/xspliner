...
Family stores information about the distribution of response - standard one is gaussian, what assumes that response comes from normal distribution. For classification binomial family is used. 

Link parameters stores info about what function should be used to transform response. The transformation is used in final model fitting. The standard link is identity (for gaussian distribution) - for binomial distibution logit is used.

See more at `??stats::family.glm`.

`xspline` function allows you to decide which response should be used in final model.
Let's check below example in which poisson distribution with log link is used.
...
Let's check below example with random forest model:
...
In fact this is not obligate. How can it be used? For example to build simpler model based on truncated amount of predictors.
Let's see below example:
...
As we can see Wind and Temp variables are of highest importance. Let's build xspliner basnig on just the Two variables.
```{r message = FALSE}
