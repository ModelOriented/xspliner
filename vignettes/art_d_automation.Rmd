...
This document present how can you customize xspliner usage to you make it more easier and automate.

# Hierarchy of parameters

## Local transition and effect

In previous sections we learned how to specify parameters for effect and transition of single variable.
Let's name them **local** parameters.

Quick example you can see below:
```{r message = FALSE}
library(xspliner)
...
When black box model is based on higher amount of variables it can be problematic to specify local parameters for each predictor. Also formula becomes large and hardly to read.

## Global transition and effect

Its more common that we use similiar configuration for each variable, as it's simple and allows us to build and experiment faster. To do this you can specify `xs_opts` and `xf_opts` of `xspline` function.
...
But still you can specify local parameters that overrides the global ones.
...
In this case `last_evaluation` variable will be transformed with thin plate regression spline (`bs = "tp"` is default for `mgcv::s`) with basis dimension equal to `10`. At the same time `average_montly_hours` will be transformed with cubic splines.
...
```
But having model based on huge amount of variables it still requires a lot of our work to build the final model.
...
The parameter is used to specify if predictor for which `xs` or `xf` was specified needs to be transformed or used as a bare variable in formula. You can specify it in local or global transition parameter. In this case using global is more reasonable.
...
Even having above learned options, we still can have problems with handling model with huge amount of variables.
...
`xspline` function tries to establish data in following order (if some way is not possible, it tries the next one):

- model formula predictors
- model training data predictors
- xspline provided data parameter, excluding formula response

To assure correct predictors usage, you may also specify precitor names vector in `predictors` parameter, and data (optional) to assure source of variable classes:
```{r message = FALSE}
model_xs <- xspline(Petal.Width ~ ., 
                    model = rf_iris,
...
Unfortunately this simplicity works thanks to few approaches that randomForest follows.

They are:

- Building the black box with formula
- Black box training data stores factors in long form (the factor is one column, not spreaded into binary wide form)

Most of R based ML models satisfies above condition. One of exceptions is XGBoost which in actual xspliner version needs to be handled in more complex xspline call. You can see it in [use cases](https://krystian8207.github.io/xspliner/articles/use_cases.html) 

## Exluding predictors from transformation
...
As we can see `rad` variable is integer and has only 9 unique values. As a result spline approximation may be misleading, and not possible to perform. We decide here to omit `rad` variable when performing transformation, nevertheless reamining predictors should be transformed.
...
As we can see in summary, rad variable was omitted during transformation.
...
As mentioned in previous section, xspliner is integrated with DALEX package.
...
You can provide your own xspline's parameters that overwrites that sourced from explainer.
