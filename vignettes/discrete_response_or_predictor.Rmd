...
As before let's explain the approach basing on random forest model. 
...
and build regression tree, when we predict average_montly_hours based on last_evaluation, salary and satisfaction_level predictors.
...
Similiarily to continuous variable it is enough to use `xf` symbol on `salary` variable, ie. use formula:
```
average_montly_hours ~ last_evaluation + xf(salary) + satisfaction_level
```

We could use the formula for our final GLM model (the we will use basic parameters for xs), but first let's learn what changes it does.

## Individual Conditional Expectation for black box model

Similiarily for continuous variables the first thing we do is to get black box model response on signle variable, call `effect`.

For continuous variable we used PD or ALE plots, which was average model response on prodictor value.
In discrete case we use Individual Conditional Expectation (ICE). The construction is simple:
...
In xspliner, we get result with `pdp::partial(ice = TRUE)` function. 
To specify additional options for the response, we may customize effect parameter for `xf`, just like it was using xs:

```
average_montly_hours ~ last_evaluation + xf(salary, effect = list(...)) + satisfaction_level
```
...
## Merging similiar groups with factorMerger

In countinuous case, we simplified the effect with spline approximation. The main idea for discrete case is to find out which groups gives similiar black box model response and merge them into common groups. 
The final model is build on simpler variables that store some information sourced from black box.
...
As in countinuous case we need to use the formula withing `xspline` function:
...
As the final GLM model can work only with binary classification, the only limit here is the number of levels for predicted value (equal to 2).
...
When base model response variable is of class factor (or integer with two unique values) then xspliner automatically detects classification problem. To force specific model response distribution you can set family and link parameters. 
