...
*Remark*
Here we ca see that response function is presented as data.frame with two columns:
...
Let's name approximation result as **transition**. To specify it's parameters such as spline base we can use `transition` parameter inside `xs` symbol. Similiarily to `effect`, `transition` is specified as parameters list. Possible options can be found by `??mgcv::s` (few extra options can be found in [extras](https://krystian8207.github.io/xspliner/articles/extras.html)).
...
The final step is to use `xspline` function (from `xspliner` package) to build the desired model.
...
- [Discrete response or predictor](https://krystian8207.github.io/xspliner/articles/discrete_response_or_predictor.html) - learn how model works when black box is classificator or what effect and transition is used when predictor is dicrete variable
- Automation - learn how to automate building xspliners without specifying your own formula and/or using global parameters
- Graphics - learn how to plot the results, or compare compare xplsiner with base black box.
